...
const InscricaoForm = ({ clube, admin, ismaster }) => {
    ...
    const [inscricaoBloqueada, setInscricaoBloqueada] = useState(false);
        ...
        const unsubscribe = onSnapshot(eventosRef, (snapshot) => {
            const lista = snapshot.docs.map((doc) => {
                ...
                return {
                    ...
                    clubes: data.clubes || [],
                    isMab: data.isMab || false,
                };
            });

            const eventosMab = lista.filter((e) => e.isMab);
            const eventosNaoMab = lista.filter((e) => !e.isMab);

            const inscritoEmOutroEventoNaoMab = eventosNaoMab.some(
                (evento) => evento.inscritos.includes(clube)
            );

            const eventosOrdenados = eventosMab.sort((a, b) => {
            ...
            setInscricaoBloqueada(inscritoEmOutroEventoNaoMab);
        });
    ...
    const handleInscricaoDireta = async (torneioId) => {
        ...
        if (inscricaoBloqueada) {
            alert("Seu clube já está inscrito em outro evento e não pode se inscrever em uma Apresentação Especial.");
            return;
        }
        ...
        <div className="event-list-section">
            ...
            {inscricaoBloqueada && (
                <div className="alerta-bloqueio">
                    ⚠️ Seu clube já está inscrito em outro evento e não pode participar de uma Apresentação Especial.
                </div>
            )}
                        ...
                        <div className="membros-area">
                            <table className="table">
                                <tbody>
                                    ...
                                    {(ismaster || t.inscritos?.includes(clube)) && (
                                        <>
                                            <tr>
                                                <th colSpan={2}>Clubes Inscritos</th>
                                            </tr>
                                            {t.inscritos.map((c, idx) => (
                                                <tr key={idx}>
                                                    <td colSpan={2}>{c}</td>
                                                </tr>
                                            ))}
            ...
            {!inscricaoBloqueada && torneiosInscritos.length < limiteDeInscricoes && (
                <div className="teste">
